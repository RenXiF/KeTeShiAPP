<template>
	<view class="login">
		<view class="content">
			<!-- 头部logo -->
			<view class="header">
				<image :src="logoImage"></image>
			</view>
			<!-- 主体表单 -->
			<view class="main">
				<wInput v-model="phoneData" type="text" maxlength="11" placeholder="电话"></wInput>
				<wInput v-model="passData" type="password" maxlength="20" placeholder="密码"></wInput>
			</view>
			<wButton text="登 录" :rotate="isRotate" @click.native="startLogin()" class="wbutton"></wButton>

			<!-- 其他登录 -->
			<!-- <view class="other_login cuIcon">
				<view class="login_icon">
					<view class="cuIcon-weixin" @tap="login_weixin"></view>
				</view>
				<view class="login_icon">
					<view class="cuIcon-weibo" @tap="login_weibo"></view>
				</view>
				<view class="login_icon">
					<view class="cuIcon-github" @tap="login_github"></view>
				</view>
			</view> -->

			<!-- 底部信息 -->
			<view class="footer">
				<view class="privacyck flex-center flex_jufy_center">
					<view class="privacy" :class="[privacy?'privacyShow':'']" @click="privacy = !privacy"></view>
				</view>
				
				<navigator url="../user/aboutApp/privacyPolicy" open-type="navigate">服务条款及隐私政策</navigator>
				<text>|</text>
				<navigator url="../user/userBinding" open-type="navigate">注册账号</navigator>
			</view>
			<view class="the_log flex-center flex_jufy_center">
				<image :src="image" mode="widthFix" @click="loginweixin()"></image>
			</view>
			
		</view>
	</view>
</template>

<script>
	var _this;
	import wInput from '../../components/watch-login/watch-input.vue' //input
	import wButton from '../../components/watch-login/watch-button.vue' //button

	export default {
		data() {
			return {
				//logo图片 base64
				logoImage: 'data:image/png;base64,',
				image:'data:image/png;base64,',
				phoneData: '', //用户/电话
				passData: '', //密码
				privacy:false,
				isRotate: false, //是否加载旋转
				onlad:{
					login:false,
					openid:''
				}
			};
		},
		components: {
			wInput,
			wButton,
		},
		mounted() {
			_this = this;
		},
		onLoad(e) {
			console.log(e);
			// this.onlad = e;
			if(e.login === "true"){
				this.loginWeiXin(e.openid);
			}
		},
		onShow() {
			
		},
		methods: {
			//手机号登录
			loginphone() {
				this.http.getApi('/user/loginPhone',{userPhone:this.phoneData, password:this.passData}, 'post').then(res => {
					console.log("成功");
					console.log(res);
					this.utils.success("登录成功！",function(){
						_this.isRotate = false;
						uni.setStorageSync('openid', res.data.userPhone);
						uni.setStorageSync('userlist', res.data);
						uni.switchTab({url: '/pages/user/user'});
					});
				}).catch(err => {
					console.log(err);
					_this.isRotate = false;
				});
			},
			//微信验证登录
			loginWeiXin(openid) {
				console.log(openid);
				this.http.getApi('/user/login',{storeOpendid:openid}, 'post').then(res => {
					console.log("成功");
					console.log(res);
					uni.hideLoading();
					this.utils.success("登录成功！",function(){
						uni.setStorageSync('WXopenid', res.data.userWxid);
						uni.setStorageSync('userlist', res.data);
						uni.switchTab({url: '/pages/user/user'});
					});
				}).catch(err => {
					console.log(err);
					uni.hideLoading();
					_this.doUrl("pages/user/userBinding");
				});
			},
			// 微信登录
			loginweixin() {
				if(!this.privacy){
					this.utils.error('请先同意服务条款');
					return;
				}
				this.utils.showloading();
				uni.login({
					provider: 'weixin',
					success: function(loginRes) {
						// console.log(loginRes.authResult);
						// 获取用户信息
						uni.getUserInfo({
							provider: 'weixin',
							success: function(infoRes) {
								uni.setStorageSync('WXopenid', infoRes.userInfo.openId);
								uni.setStorageSync('WXuser', infoRes.userInfo);
								console.log(infoRes.userInfo);
								_this.loginWeiXin(infoRes.userInfo.openId);
							}
						});
						uni.hideLoading();
					},
					fail:function(err){
						console.log(err);
						if(err.code == -2 || err.code == -100){
							_this.utils.error('您已取消授权！');
							_this.isRotate = false;
						}
					}
				});
			},
			startLogin() {
				//登录
				if (this.isRotate) {
					//判断是否加载中，避免重复点击请求
					return false;
				}
				if (this.phoneData.length == "" || this.utils.checkMobile(this.phoneData)==false) {
					uni.showToast({
						icon: 'none',
						position: 'bottom',
						title: '手机号格式不正确'
					});
					return;
				}
				if (this.passData.length < 5) {
					uni.showToast({
						icon: 'none',
						position: 'bottom',
						title: '密码不正确'
					});
					return;
				}
				if(!this.privacy){
					this.utils.error('请先同意服务条款');
					return;
				}
				_this.isRotate = true;
				setTimeout(function() {
					_this.loginphone();
				}, 500);

			}
		}
	}
</script>

<style lang="scss">
	@import url("../../components/watch-login/css/icon.css");
	@import url("./css/main.css");
	.the_log{
		width: 100%;
		margin-top: 50px;
		image{
			width: 60px;
			// border: #007AFF 1px solid;
		}
	}
	/*  */
	.privacyck{
		width: 30rpx;
		height: 30rpx;
		margin-right: 10rpx;
		border-radius: 50%;
		border: #3C3C3C 1rpx solid;
	}
	.privacy{
		width: 85%;
		height: 85%;
		border-radius: 50%;
		// border: #3C3C3C 1rpx solid;
	}
	.privacyShow{
		// padding: -30rpx;
		// border: #2C405A 2rpx solid;
		background-color: #3C3C3C;
	}
</style>
