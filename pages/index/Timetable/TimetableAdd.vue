<template>
	...
	<view class="index_feedbk">
		<navigation @clickLeft="navback()" @clickRight="Submit()" fixed="true" title="添加课表" backgroundColor="#ffffff"
		 leftIcon="../../../static/icon/left.png" rightText="发布"></navigation>
	...
	<script>
		...
		export default {
			...
			methods:{
				...
				Submit() {
					if (this.fromlist.imageOne== '') {
						this.utils.error('请至少上传一张图片');
						return;
					}
					console.log(this.fromlist)
					this.utils.showloading();
					this.http.getApi('Table/Add', this.fromlist, 'post').then(res => {
						console.log("res");
						console.log(res);
						...
						console.log(err);
						if(err.status == 13){
							this.utils.error('当前课表已存在');
							this.utils.showloading('正在修改课表');
							this.timetableData();
						}else{
							uni.hideLoading();
				...
				xiugai(){
					this.http.getApi('Table/update', this.fromlist, 'post').then(res => {
						console.log("res");
						console.log(res);
