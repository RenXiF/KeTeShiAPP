	...
	<view class="semp-notice-bar">
		<view class="show-box flex-row" :class="round?'round':''" v-show="showNotice" :style="{'background':bgColor}">
			...
			<view class="show-info flex-row" :class="icon==''?'no-icon':''" v-if="showType=='slider'">
				<view class="text-box" :class="rows && !scrollable?'text-rows':''" :style="{'margin-left':sw+'px','color':color}"
			...
			<view class="scroll-box" :style="{'color':color}" v-if="showType=='scrollTop'">
				<swiper vertical="true" autoplay="true" circular="true" interval="3000">
					<swiper-item v-for="(item, index) in arrayText" :key="index">
						<navigator @click="onItemClick" v-if="item.proclamationName" class="flex-between">{{item.proclamationName}}<text style="font-size: 12px; color: #CBCBCD;">{{item.createdTime.substring(0,16)}}</text></navigator>
						<navigator v-else>暂无通知</navigator>
					</swiper-item>
				</swiper>
			</view>
